...
In principle, you should be able to replace the .bit file in the /boot directory of the .wic image referenced above with the one generated here and it should work. When I initally tried this, it locked up the system. See the probable reason in the next paragraph. Since fixing the AXI issue, I have not gone back to see if it works with the original .wic image.
...
Once the server is built, tar.gz the server code (which now has the client as well) and transfer it to the Zynq. Create a `/home/socdp8` directory and change the owner to petalinux. You should now be able to run the server `sudo node lib/main.js`, assuming you use the client and server code in the 2025.1 branch. While some stuff seems to work, other stuff does not. For example if you load a snippet and read it back with `EXAM`, you don't read back the correct values.

It turns out the readback issue is due to Vivado's auto connect not handling the BRAM controller well. To fix this, you can in Vivado source the `patches/BRAM_Adapter.tcl` file and then at the tcl prompt enter `create_hier_cell_BRAM_Adapter pdp8i BRAM_Adapter`. Wire the new component up between the BRAM and controller as illustrated in `patches/pynq_z2_block.pdf`. Rebuild the bitstream, export the xsa and then reimport it into the Petalinux project and rebuild that. With this change I was able to load the blinking light snippet, confirm memory read back fine and run the snippet.

Getting to this point was not easy. Documentation is fragmented, unclear, often out of date and sometimes wrong. There are a lot of pieces involved. So it is entirely possible, I've missed things in this description. If there is something wrong or unclear, raise it as an issue and I'll try and fix it. Similarly, if there are better ways to do any of this, I'd love to hear about it.
