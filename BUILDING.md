...
It seems that some small changes need to be made to the server to allow the client to work. Basically, you must use https not http, and there are some other small changes as well. The server code has been updated in the 2025.1 branch here and runs. The server needs to run with elevated privilege, so sudo node lib/main.js to start it.
